...
bool ArchivoPersistencia::guardarUsuarios(const SistemaUsuarios &sistemaUsuarios, const string &rutaArchivo) {
    ...
    for (int i = 0; i < sistemaUsuarios.totalUsuarios; i++) {
        ...
        uint8_t recompensaFinalProgresiva = usuario->recompensaFinalProgresivaReclamada ? 1 : 0;

        if (!escribirValor(archivo, musica) ||
            !escribirValor(archivo, recompensaSecreta) ||
            !escribirValor(archivo, recompensaFinalProgresiva)) {
...
bool ArchivoPersistencia::cargarUsuarios(SistemaUsuarios &sistemaUsuarios, const string &rutaArchivo) {
    ...
    for (uint32_t i = 0; i < total; i++) {
        ...
        uint8_t recompensaFinalProgresiva = 0;

        if (!leerValor(archivo, musica) ||
            !leerValor(archivo, recompensaSecreta) ||
            !leerValor(archivo, recompensaFinalProgresiva)) {
        ...
        usuario->recompensaFinalProgresivaReclamada = recompensaFinalProgresiva != 0;
